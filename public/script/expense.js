...
function leaderboard(e){
    if(e.target.classList.contains('leaderboard')){
        ...
        const response = await axios.get('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/leaderboard', {headers:{"authorization": token}})
...
document.getElementById("rzp-button1").onclick = async function(e){
    const response = await axios.get('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/premium', {headers:{"authorization": token}})
    console.log(response)
     ...
     var options ={
        ...
        "handler": async (response)=>{
            await axios.post('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/premium',{
    ...
    rzp1.on('payment.failed', async function(response){
        await axios.post('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/paymentfailure',{
                order_id: options.order_id,
            },{headers:{"authorization": token}})
...
if(premiumUser==="true"){
   ...
   premiumFeatureDiv.appendChild(weeklyButton)
    ...
    if(e.target.classList.contains('downloadButton')){
        axios.get('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/download', {headers:{"authorization": token}})
        .then((response) => {
...
let pageno = 1;
axios.get(`http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/expense?pageno=${pageno}`, {headers:{"authorization": token, "pageno":pageno}})
    ...
    if(formButton.classList.contains('SubmitButton')){
        myObj = {
           ...
           // authorization: token
            ...
            axios.post('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/expense',{myObj},{headers:{"authorization": token} })
            .then(e=> {
        ...
        axios.put('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/expense',myObj).then(e=> location.reload()).catch(err => console.log(err))
...
axios.delete('http://ec2-54-91-147-182.compute-1.amazonaws.com:4000/expense',{ data: {  id: li.firstChild.textContent } }  )
  .then(e =>{location.reload()})
  .catch(err=> console.log(err)); 
 //   
}}
